...
function updateCameraPosition() {
  ...
  const lookAtPoint = shipPosition
    .clone()
  ...
  const cameraPosition = shipPosition
    .clone()
    .add(backwardDirection.multiplyScalar(1.5));
...
function spawnApproachEncounter(distance, encounterIndex) {
  ...
  console.log(
    `Spawning approach encounter at distance ${distance} with ${actualNumToSpawn} TIE Fighters`
  );
...
function spawnTieFighters() {
  ...
  if (
    currentTime - lastTieFighterSpawnTime > TIE_FIGHTER_SPAWN_INTERVAL &&
    tieFighters.length < 10
  ) {
...
function updateTieFighters() {
  ...
  for (let i = tieFighters.length - 1; i >= 0; i--) {
    ...
    directionToPlayer
      .subVectors(xwing.position, tieFighter.mesh.position)
      .normalize();

    // Calculate distance to player
    const distanceToPlayer = tieFighter.mesh.position.distanceTo(
      xwing.position
    );
    ...
    if (tieFighter.behaviorTimer <= 0) {
      ...
      if (tieFighter.behaviorState === "pursue" && distanceToPlayer < 25) {
        ...
        tieFighter.behaviorTimer =
          tieFighter.behaviorState === "evade"
    ...
    // Apply movement
    tieFighter.mesh.position.add(
      moveDirection.multiplyScalar(tieFighter.speed)
    );
    ...
    if (
      (tieFighter.behaviorState !== "evade" || Math.random() < 0.1) && // 10% chance to fire even when evading
      distanceToPlayer < 60 && // Increased from 50 to 60
      currentTime - tieFighter.lastFireTime > tieFighter.fireInterval * 0.8
    ) {
      // 20% faster firing
